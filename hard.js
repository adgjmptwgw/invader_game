...
$('#startbtn').on('click', function () {
  ...
  //G's消去タイマー
  setTimeout(gsDelete, 29000);

  //勝ち判定タイマー
  setTimeout(alertmsg_win, 97000);

  //宝箱メニューアラートの出現時間設定
  setTimeout(jankentimer, 2000); 
  setTimeout(jankentimer, 9000); 
  setTimeout(jankentimer, 14000); 
  setTimeout(jankentimer, 24000); 
...
//40体分の体力  ※敵の体力ゲージを定義
let physical_gage = [6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6];
// let invaders = []; 
...
//G'sを打つと負ける
$('body').on('click', function () {
  setTimeout(gstimer, 250);
});

let gstimer = function () {
  let myself = $('.plane').offset();
  let myPosi = myself.left;
  let gs = $('.gs').offset();
  let GsPosi = gs.left;
